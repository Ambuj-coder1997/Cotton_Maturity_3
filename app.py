...
import gdown  # Ensure gdown is listed in your requirements.txt
...
# Extracted file ID from your provided Google Drive link.
...
if uploaded_file is not None:
    ...
    # Run inference: get classification and bounding-box predictions.
    class_pred, bbox_pred = model.predict(image_array)
    class_idx = np.argmax(class_pred[0])
    confidence = class_pred[0][class_idx]
    
    # Debugging: Show prediction values.
    st.write("Prediction confidence:", confidence)
    st.write("Raw classification prediction:", class_pred)
    st.write("Raw bounding box prediction:", bbox_pred)
    
    if confidence < conf_threshold:
         st.write("Detection confidence below threshold.")
    else:
         # Convert normalized bounding box [cx, cy, w, h] to image-scale coordinates.
         ...
         # Debugging: Show computed bounding box coordinates.
         st.write("Computed bounding box:", (x1, y1, x2, y2))
         ...
         st.image(image_cv, caption="Detection Results", use_container_width=True)
